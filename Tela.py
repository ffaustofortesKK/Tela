...
# Limpeza de segurança caso a TV carregue num estado fantasma
...
# ==========================================
# 1. ESTADO: CANTAR (KARAOKE ECRÃ INTEIRO)
# ==========================================
    ...
    <body>
        ...
        <script>
            ...
            video.onended = function() {{
                video.pause();
                document.getElementById('app').innerHTML = "<h1 style='color:white;'>Encerrando...</h1>";
                fetch('{URL_STATUS}', {{
                    method: 'PUT',
                    headers: {{ 'Content-Type': 'application/json' }},
                    body: JSON.stringify({{"comando": "parar", "cantor": "", "musica": "", "url_video": "", "id_sessao": "limpo"}})
                }}).then(() => {{
                    window.parent.location.reload(); // Recarrega a TV inteira!
                }});
            }};
...
# ==========================================
# 2. ESTADO NORMAL: FILA + CLIPE DE FUNDO
# ==========================================
else:
    ...
    with cl2:
        ...
        if comando == "clipe" and url_video:
            ...
            # Aqui está o player com a tranca anti-loop absoluto
            mini_player_html = f"""
            <!DOCTYPE html>
            <html>
            <body style="margin:0; background:black; overflow:hidden;">
                ...
                <script>
                    ...
                    v.loop = false;
                    v.muted = true; // Para o clipe tocar de fundo sem bloquear o autoplay do browser

                    // TRANCA ANTI-LOOP: Se este vídeo já tocou nesta sessão, bloqueia imediatamente.
                    if (sessionStorage.getItem('tocado_clipe_' + sessaoAtual)) {{
                        document.body.innerHTML = "<div style='color:gray;text-align:center;padding-top:40%;font-family:sans-serif;'>Limpando player...</div>";
                        fetch('{URL_STATUS}', {{
                            method: 'PUT',
                            headers: {{ 'Content-Type': 'application/json' }},
                            body: JSON.stringify({{"comando": "parar", "cantor": "", "musica": "", "url_video": "", "id_sessao": "limpo"}})
                        }}).then(() => {{
                            window.parent.location.reload(); // Recarrega a TV (Pai)
                        }});
                    }} else {{
                        v.autoplay = true;
                        v.play().catch(e => console.log("Erro de autoplay:", e));
                    }}

                    // Quando o clipe acaba naturalmente
                    v.onended = function() {{
                        v.pause();
                        
                        // Marca no browser que esta sessão terminou (bloqueia o loop)
                        sessionStorage.setItem('tocado_clipe_' + sessaoAtual, 'true');
                        
                        document.body.innerHTML = "<div style='color:gray;text-align:center;padding-top:40%;font-family:sans-serif;'>Fim do clipe. Atualizando...</div>";

                        // Manda parar para o Firebase
                        fetch('{URL_STATUS}', {{
                            method: 'PUT',
                            headers: {{ 'Content-Type': 'application/json' }},
                            body: JSON.stringify({{"comando": "parar", "cantor": "", "musica": "", "url_video": "", "id_sessao": "limpo"}})
            ...
            # Script que fica à escuta do Firebase quando a TV está parada
            espera_ativa_html = f"""
            <script>
                setInterval(() => {{
                    fetch('{URL_STATUS}?nocache=' + new Date().getTime())
                        .then(res => res.json())
                        .then(data => {{
                            // Se receber um comando novo e não for 'parar'
                            if (data && data.url_video && data.comando && data.comando !== 'parar') {{
                                window.parent.location.reload(); // ATUALIZA A TV PAI PARA LER O NOVO COMANDO
                            }}
                        }});
                }}, 2500);
            </script>
